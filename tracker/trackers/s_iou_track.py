...
class S_Ioutrack(object):
    def __init__(self, args, frame_rate=30):
        ...
        self.frame_id = 0
        self.args = args
        print("args.conf_thresh:", args.conf_thresh)
    ...
    def update(self, output_results, img, ori_img):
        ...
        # 2024年12月10日查看score的分布情况
        # print("scores:", scores)
        # 一行计算所有统计值
        # median, mode_result, min_val, max_val, q1, q3, std_dev, mean_val = np.median(scores), stats.mode(scores), np.min(scores), np.max(scores), np.percentile(scores, 25), np.percentile(scores, 75), np.std(scores), np.mean(scores)

        # 打印所有结果
        # print(f"中位数: {median}, 众数: {mode_result.mode}, 出现次数: {mode_result.count}, 值的分布范围: [{min_val}, {max_val}], 第一四分位数 (Q1): {q1}, 第三四分位数 (Q3): {q3}, 标准差: {std_dev}, 均值: {mean_val}")
        # print("output:", output)
        # 筛选检测结果,根据置信度分数筛选检测框：remain_inds 表示高于阈值的检测框。inds_second 用于表示低置信度的检测框。
        # 中等置信度目标框inds_second：0.1 < scores < self.args.conf_thresh。
        # 高置信度目标框remain_inds：scores > self.args.conf_thresh。
        # print("s_iou_track args.conf_thresh:", self.args.conf_thresh) # 2025年7月23日
        ...
        # 创建检测对象 根据检测框和特征创建相应的跟踪目标（Tracklet 或 Tracklet_w_reid）。 dets = bboxes[remain_inds] 对应高置信度
        # 进行匹配，score_keep中的置信度维度和下面的维度相互对应
        if len(dets) > 0:
            # print("self.with_reid:", self.with_reid) # 2025年7月23日注释
            if self.with_reid:
        ...
        # print("detecttions:", detections)   2025年7月23日
        ...
        #print("tracklet_pool:", tracklet_pool, "detections:", detections)
        # print("ious_dists:", ious_dists)
        # 2025年7月23日 把一些中间的调试记录都注释了  
        # print("tracklet_pool.shape:", len(tracklet_pool), ",detections.shape:", len(detections))
        # print("ious_dists.shape:", ious_dists.shape)
        # print("type(ious_dists), type(scores_keep):", type(ious_dists), type(scores_keep))

        

        # 2024年12月12日 注释原来的  然后再命令行中设置conf_thresh为0.2,然后将下面的ious_dists改成updated_ious_dists
        # 注意我们这里只对第一阶段的ious_dists进行加权了
        ...
        # 2024年12月12日 确保置信度越高的值对更新影响越大，同时整体变化控制在合理范围内
        # 计算权重
        weights = scores_keep / scores_keep.sum()
        # 广播加权更新   注意是减去对应的值 因为距离越小越好
        # updated_ious_dists = ious_dists + weights * scores_keep  # 更新 2025年7月21日
        updated_ious_dists = ious_dists   # 不更新 2025年7月21日 速度消融 ，若不测试需要注释
        # updated_ious_dists = ious_dists - weights * scores_keep

        # print("updated_ious_dists1")
        # ious_dists_mask = (updated_ious_dists > 0.5)  # high conf iou 原始
        ious_dists_mask = (updated_ious_dists > 0.4)  # high conf iou # 2025年5月27日改成0.4
        # 计算嵌入距离 如果启用了再识别，计算嵌入距离，并根据 IOU 和嵌入距离更新距离矩阵。
        if self.with_reid:
            # mixed cost matrix
            emb_dists = embedding_distance(tracklet_pool, detections) / 2.0
            ...
            emb_dists[emb_dists > 0.25] = 1.0
            emb_dists[ious_dists_mask] = 1.0
            dists = np.minimum(updated_ious_dists, emb_dists)

        else:
            dists = updated_ious_dists
        ...
        # 第二步：低分检测框的关联
        # print("len(dets_second):", len(dets_second))
        # if len(dets_second) > 0:
        #     '''Detections'''
        #     print("low score detections")
        ...
        # 2024年12月12日
        # print("len(dets_second):", len(dets_second))  # 2025年7月23日
        if len(dets) > 0:
            # print("self.with_reid:", self.with_reid)
            if self.with_reid:
        ...
        # 2024年12月9日尝试对低分的框进行改进
        num_iteration = 2
        init_expand_scale = 0.2 # 将0.7改成0.25 0.10试试
        expand_scale_step = 0.1
        cur_expand_scale = init_expand_scale
        # 2024年12月9日
        # print("eiou:")
        
        dists = iou_distance(r_tracked_tracklets, detections_second)
        # print("dists:", dists)
        # dists = eiou_distance(r_tracked_tracklets, detections_second, cur_expand_scale)
        # print("edists:", dists)
        ...
        for itracked, idet in matches:
            ...
            if track.state == TrackState.Tracked:
                # print("track type:", type(track)) # 2025年7月23日
        ...
        # ious_dists = iou_distance(unconfirmed, detections) # 2024年12月10日
        ious_dists = eiou_distance(unconfirmed, detections, 0.5)
        # print("no match:", ious_dists)
