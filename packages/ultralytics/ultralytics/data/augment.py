...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        img_2 = labels.get("img_2") if image is None else image

        # print("在if img.shape[2] == 6 之前 img shape:", img.shape, "img_2 shape:", img_2.shape) 
        # 以上打印结果为：    在if img.shape[2] == 6 之前 img shape: (765, 1360, 6) img_2 shape: (765, 1360, 6)
        # 此时两张图片维度还是一致。
        ...
        if img.shape[2] == 6:
            img = img[..., :3]
            img_2 = img_2[..., 3:]
        # print("在if img.shape[2] == 6 之后 img shape:", img.shape, "img_2 shape:", img_2.shape) 
        # 如下，此时结果也是一致的，在if img.shape[2] == 6 之后 img shape: (765, 1360, 3) img_2 shape: (765, 1360, 3)
        ...
        # print("在if self.center之后， img shape:", img.shape, "img_2 shape:", img_2.shape) 
        # 在这里尺寸仍然一致：    在if self.center之后， img shape: (765, 1360, 3) img_2 shape: (765, 1360, 3)

        # print("shape[::-1]:", shape[::-1], "new_unpad:", new_unpad)
        if shape[::-1] != new_unpad:  # resize
            img = cv2.resize(img, new_unpad, interpolation=cv2.INTER_LINEAR)
            img_2 = cv2.resize(img_2, new_unpad, interpolation=cv2.INTER_LINEAR)
        top, bottom = int(round(dh - 0.1)) if self.center else 0, int(round(dh + 0.1))
        left, right = int(round(dw - 0.1)) if self.center else 0, int(round(dw + 0.1))
        img = cv2.copyMakeBorder(
            img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)
        )  # add border
        # 2024年12月26日 增加这一部分后，尺寸就一样了。
        img_2 = cv2.copyMakeBorder(
            img_2, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)
        ) # add border

        # print("copyMakeBorder 之后 img shape:", img.shape)
        ...
        # z注意在这里两个尺寸打印出来不一样
        # print("augment.py img shape:", img.shape, "img_2 shape:", img_2.shape) # img shape: (736, 1280, 3) img_2 shape: (765, 1360, 3)

        # 其中一次打印结果  img shape: (736, 1280, 3) img_2 shape: (765, 1360, 3)
        if len(labels):
            labels = self._update_labels(labels, ratio, dw, dh)
            labels["img"] = img
            labels["img_2"] = img_2 # 2024年12月26日
            labels["resized_shape"] = new_shape
            return labels
        else:
            if image.shape[2] == 6:
